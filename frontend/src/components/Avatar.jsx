...
export default function Avatar({ active, ready = true }) {
  ...
  const rotateY = useTransform(sx, (v) => v * 42); // whole body turns toward cursor
  ...
  useEffect(() => {
    ...
    // idle drift: gently leans/looks around on its own when the cursor is still
    let raf;
    const loop = () => {
